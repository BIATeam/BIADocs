...
3. Delete all package-lock.json
...
4. If you were using the site domain, move it out of the bia-domain folder and correct the import paths:
**src/app/domains/bia-domains/site-option** => **src/app/domains/site-option**
...
3. With VSCode, Enable search via regex and fill out the search form like this:
  
  Search: ```.FirstName \+([^*]*).LastName```

  Replace: ```.LastName +$1.FirstName```

  files to include: ```*Mapper.cs```

  And click on **Replace All**

4. Search LastName in all mapper in EntityToDto function
use user.Display() instead of 
user.LastName + " " + user.FirstName + " (" + user.Login + ")"
And user.DisplayShort() instead of 
user.LastName + " " + user.FirstName

5. HandlerRepository change Constructor base parameter:
...
6. In **Crosscutting.Ioc.IocContainer.ConfigureApplicationContainer**, If you have this line of code twice, delete the one that is not contained in the **if (isApi)**
  ```csharp
...
7. If you used **SortField** on the BackEnd side, replace it with **MultiSortMeta**
