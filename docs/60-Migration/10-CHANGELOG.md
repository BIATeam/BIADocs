...
* Improve `BiaHybridCache` service
* Token bearer content improvement :
  * Transform all previous constants defined into `Rights.cs` to enum values into `PermissionId.cs`
  * All permissions stored in token are now numeric identifiers instead of string values
  * Permissions previously stored into `http://schemas.microsoft.com/ws/2008/06/identity/claims/role` token property are now stored into `urn:bia:claims:permission_ids`
  * Roles identifiers previously stored into `http://schemas.microsoft.com/ws/2008/06/identity/claims/roleid` token property are now stored into `urn:bia:claims:role_ids`
  * Mapping between permission string and enum values are retrieved by the front-end from the application settings request at startup
### Bugfix
* Fix issues with bearer token too large
* Fix errors generated by `bia-table-header`
...
* New team configuration
  ...
  * Move team additional configuration from front-end `all-environments.ts` to back-end ([doc](../10-Introduction/30-TryIt/40-CreateAirlineTeam.md))
...
* Add new `BiaRowVersionProperty` attribute for properties that should be considered as the dedicated row version column of entities that inherits from a parent versioned entity ([example doc for Teams](../10-Introduction/30-TryIt/40-CreateAirlineTeam.md))
...
* Configuration to clear team and have no team selected ([doc](../10-Introduction/30-TryIt/40-CreateAirlineTeam.md))
* Configuration to choose automatic team selection mode if no default team set ([doc](../10-Introduction/30-TryIt/40-CreateAirlineTeam.md))
