...
1. Run command `ng update @angular/core@19.2.15 @angular/common@19.2.15 @angular/compiler@19.2.15 @angular/forms@19.2.15 @angular/cli@19.2.18 @angular/platform-browser@19.2.15 @angular/platform-browser-dynamic@19.2.15 @angular/router@19.2.15 @angular/service-worker@19.2.15 @angular/build@19.2.18 @angular/animations@19.2.15 @angular/compiler-cli@19.2.15 @angular-eslint/schematics@19.8.1 @angular-eslint/eslint-plugin@19.8.1 @angular-eslint/eslint-plugin-template@19.8.1 @angular/cdk@19.2.19 @ngrx/store@19.2.1 @ngrx/store-devtools@19.2.1 keycloak-angular@19.0.2 primeng@19.1.3 @microsoft/signalr@8.0.7`, you'll have the following automatic migrations :
   1. From **@angular/cli**
...
8. Relaunch BIAToolKit and reload your project on **Modify existing project** tab, or click on **Refresh project informations** button if it still opened
...
15. Customize your application logo ([doc](../50-BestPractices/40-Front%20Style%20Guide/40-CustomizeApplicationTheme.md#application-logo))

### Conflict resolution
You must pay attention to the conflict resolution of the following files.
...
#### Configuration files
- **ConnectionStrings** and **DBEngine** have been moved from appsettings files to bianetconfig files. You should remove these keys from your appsettings files in case of conflict.
- **EmailConfiguration** 'from' email adress has changed depending on the environment to comply with Safran mailing policy.
...
export class MyFeatureModule {}
```
22.  If previously using `<p-dialog>` with a `header` directive, move the content of the header to a `<ng-template #header>`. You can then delete the import `Header` from your TS component.
``` html title="Before" 
<p-dialog
  header="
    {{ 'feature.myHeader' | translate }}
  ">
  myContent
</p-dialog>
```
...
<p-dialog>
  <ng-template #header
    >{{ 'feature.myHeader' | translate }}</ng-template
  >
  myContent
</p-dialog>
```
23. Bia CSS variables have mostly been replaced. This a a replacement suggestion and each project should check if the colors are correct for light and dark mode.
- **$biaPrimaryColor** may be replaced by **var(--primary-color)**
- **$biaAccentColor** may be replaced by **var(--primary-color)**
- **$biaColorOnBgAccentColor** may be replaced by **#fff** (You can also create your own variable if needs be)
- **$biaBgColor** may be replaced by **var(--surface-ground)**
- **$biaMainBarColor** may be replaced by **var(--surface-ground)**
- **$biaBgHoverColor** may be replaced by **var(--surface-hover)**
- **$biaFgTextColor** may be replaced by **var(--text-color)**
- **$biaBorderColor** may be replaced by **var(--p-surface-500)**
- **$biaBgBreadcrumbColor** should not be useful anymore
- **$biaTextBreadcrumbColor** should not be useful anymore
- **$biaSurfaceColor** may be replaced by **var(--surface-ground)**
...
#### Other
- Meziantou.Framework.Win32.CredentialManager dependency has been removed from the BIA.Net.Core.Infrastructure.Service. If you still need that dependency in your project, add it in your project directly.
...
2. Edit the generated migration : 
   ...
   ");
   ```
   and move these two code blocks at the end of the Up function.
   :::warning
   Ensure to have this previous code **after** the `CreateTable` and `CreateIndex`instruction for `UserDefaultTeams` table
   :::
   ...
   5. Search for the following code into `Up()`
   ``` csharp
   migrationBuilder.AddColumn<string>(
      name: "Discriminator",
      table: "Notifications",
      ...
      defaultValue: "");
   ```
   6. Change the `defaultValue` to `Notification`
   7. Search for the following code into `Down()`
   ``` csharp
   migrationBuilder.AddColumn<bool>(
         ...
         defaultValue: false);
   ```
   8. Add after the following code :
   ``` csharp
   migrationBuilder.Sql(@"
   ...
   ```
   :::warning
   Ensure to have this previous code **before** the `DropTable` instruction for `UserDefaultTeams` table
   :::
3. Update database
