...
6. search ```this.repository.Update```
...
7. If you use the Database handler feature verifiy it is still active in bianet.config of the workerservice. Now it is stop by default.

8. Search ```useView = true``` or ```tableStateKey = '```(+ here an not empty string)
...
9. replace ```getAllSites``` by ```getAllTeamsOfType(TeamTypeId.Site)```
   and resolve using 

10. remove  ```this.store.dispatch(loadAllSites());``` or other call to ```loadAllSites``` **exept in site feature**
