...
2. Delete from your Angular projects all **package-lock.json** and **node_modules** folder
...
5. Download the [migration script](./Scripts/V4_to_V5_Replacement.ps1) and the [standalone catch up script](./Scripts/standalone-catch-up.js) into the same directory
   1. Rename the files to remove the hash of the name
   2. Change source path of the migration script to target your project root and your Angular project 
   3. Run it for each of your Angular project (change the Angular source path for each)
6. Apply other manual step [describe below](#manual-steps)
7. If all is ok, you can remove the .rej files (during the process they can be useful to resolve build problems)
8. Resolve missing and obsolete usings in back-end with BIAToolKit
9. For each Angular project, launch the **npm run clean**
...
13. In all the components extending BiaTableComponent, BiaCalcTableComponent, CrudItemTableComponent or using template file of these class, add import of BiaFrozenColumnDirective
14. Change property references of `AuthInfo.additionalInfo.userInfo.id` to `AuthInfo.decryptedToken.id`
15. Change property references of `AuthInfo.additionalInfo.userInfo.login` to `AuthInfo.decryptedToken.identityKey`
16. Replace in HTML templates of components that inherits from `CrudItemFormComponent` the reference of output `cancel` by `cancelled`
17. `<p-badge>` **value** property must not be undefined or null
18. If previously using `<p-card>` with a `<p-header>` and/or `<p-footer>`, change theses tags by `<ng-template #header>` or `<ng-template #footer>`. You can then delete the import `Header` and/or `Footer` from your TS component.
``` html title="Before" 
<p-card>
   <p-header></p-header>
   <p-footer></p-footer>
