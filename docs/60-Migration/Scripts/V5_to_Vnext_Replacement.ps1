...
function ApplyChangesToLib {
  ...
  $replacementsTS = @(
    ...
    @{Pattern = "import {(?=(?:(?!import {)[\s\S])*\bbia-ngxLoggerServerService\b)([\s\S]*?)\bbia-ngxLoggerServerService\b[,]?([\s\S]*?} from '[\s\S]*?\/bia-core\/services\/bia-ngx-logger-server\.service';)"; Replacement = 'import { bia-ngxLoggerServerService } from ''bia-ng/core''; import { $1$2'},
    ...
    @{Pattern = "import {(?=(?:(?!import {)[\s\S])*\bgetAllTeams\b)([\s\S]*?)[\s]*?\bgetAllTeams\b[,]?([\s\S]*?} from '[\s\S]*?\/bia-domains\/team\/store\/team\.state';)"; Replacement = 'import { BiaTeamsStore } from ''bia-ng/core''; import { $1$2'},
    ...
    # TODO : ADD all exported elements in case projects used them.
    ...
    @{Pattern = "import {(?=(?:(?!import {)[\s\S])*\bloadAllView\b)([\s\S]*?)[\s]*?\bloadAllView\b[,]?([\s\S]*?} from '[\s\S]*?\/bia-shared\/features\/view\/store\/views-actions';)"; Replacement = 'import { loadAllView } from ''bia-ng/shared''; import { $1$2'},
