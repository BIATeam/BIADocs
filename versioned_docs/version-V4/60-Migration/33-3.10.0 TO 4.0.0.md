...
4. Manage the conflicts (2 solutions)
   ...
   3. Note that **ConnectionStrings** name has changed in the version 4.0.2 and later of the configuration files. The correct name is now always **ProjectDatabase** in every project.

5. Change source path and run the script [V3.10.0_to_V4.0.0_Replacement.ps1](./Scripts/V3.10.0_to_V4.0.0_Replacement.ps1)

6. Apply other manual steps (described below) at the end if all is ok, you can remove the .rej files (during the process they can be useful to resolve build problems)
...
```
Replace yourTableComponentAccessor by the name of your accessor to access the bia-table and if needed, inject TableHelperService in your constructor.
...
The less buttons visible, the less confusion to the users of the application. So when possible, try to hide these icons and choose a mode for your table.

3. BiaFieldConfig, BiaFieldsConfig, CrudConfig, BiaFormComponent, CrudItemFormComponent, CrudItemService, etc. now take a generic type to better work on the correct properties of the object. The type needed is the model of the object defined in the feature and displayed in the list and in the form.
For example, the feature Plane will work with BiaFieldConfig&lt;Plane&gt; type.

4. PrimeNg sets the number of filter constraints for the table columns to a maximum of 2 by default. We changed this default maximum number to 10 and created a new property **maxConstraints** in BiaFieldConfig to change this value. Example :
```ts
...
5. A new mode for the table has been added with the V4.0.0 : Compact Mode.
...
6. An helper function has been created to help the table fill the height of the page. It needs to calculate the correct height depending on many parameters (compact mode, footer size, old layout or new layout, fullscreen, etc.). 
This change has not been automated by script as it could override a scrollHeightValue that has been previously set to a personalized value.
...
7. If you want to personalize the new layout, you can change some configuration by setting them in your app.component.ts.
...
8. Add instead of save in new view
...
9. In table header replace delete and add by icon
  search
  ```html
...
10. In your form, reverse the order of the buttons and place the primary buttons to the right:
...
11. Add icons on save and cancel in view:
...
### BACK
1. Ideally refactor your domain folders to comply with new organization
...
7. SaveSafeAsync now returns a List of dto and not a SafeSafeReturn object.
It is not necessary anymore to check for exceptions for these calls because it is now done in the application layer.
You can replace this code in error:
```csharp
...
return this.Ok();
```
8. BiaControllerBaseNoToken has been removed from Bia.Net.Core.Presentation.Api
Use AuthControllerBase from your project presentation layer instead.
