...
1. The boolean field of the **hasFilter** method of the **TableHelperService** class has been removed. deleted it if you used this method:
  this.tableHelperService.hasFilter(this.biaTableComponent, ~~true~~)

1. Search **this.setPermissions()** in all ***index.component.ts**. 
...
2. Due to a breaking change in front with Numeric value, if you have decimal number to display in some crud Table or Forms, you should configure the **displayFormat** in the model > const FieldsConfiguration.
Exemple:
```ts
    Object.assign(
