...
5. Apply other manual step [describe below](#manual-steps)
6. If all is ok, you can remove the .rej files (during the process they can be useful to resolve build problems)
7. Resolve missing and obsolete usings in back-end with BIAToolKit
8. For each Angular project, launch the following commands : 
   1.  **npm install**
   2.  **npm audit fix**
   3.  **npm run clean**
...
14. If previously using `<p-card>` with a `<p-header>` and/or `<p-footer>`, change theses tags by `<ng-template #header>` or `<ng-template #footer>`. You can then delete the import `Header` and/or `Footer` from your TS component.
``` html title="Before" 
<p-card>
   <p-header></p-header>
   <p-footer></p-footer>
